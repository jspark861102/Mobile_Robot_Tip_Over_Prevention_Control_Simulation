...
tb = [0:T:10];
dx_ref = -0.7*2*pi/10*cos(2*pi*tb/10);
dy_ref = -0.7*4*pi/10*cos(4*pi*tb/10);

tz=3;
t = [0:T:10+tz];
num=19;
dx_ref = [zeros(1,(tz*100-2*(num+1))/2) 0.7*2*pi/10*(cos(pi.*[0:1/num:1])/2-0.5) dx_ref 0.7*2*pi/10*(-cos(pi.*[0:1/num:1])/2-0.5) zeros(1,(tz*100-2*(num+1))/2)];
dy_ref = [zeros(1,(tz*100-2*(num+1))/2) 0.7*4*pi/10*(cos(pi.*[0:1/num:1])/2-0.5) dy_ref 0.7*4*pi/10*(-cos(pi.*[0:1/num:1])/2-0.5) zeros(1,(tz*100-2*(num+1))/2)];

theta_ref = [atan2(dy_ref(132),dx_ref(132))*ones(1,131)  atan2(dy_ref(132:end-131),dx_ref(132:end-131))  atan2(dy_ref(1170),dx_ref(1170))*ones(1,131)];
