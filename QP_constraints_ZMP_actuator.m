...
function [G0,E0,w0] = QP_constraints_ZMP_actuator(cur_state, xr, ddxr, x_tilt_current, x_tilt_m1, N, A, B, h2, D, L, T, switch_zmp, switch_input, thr_input, switch_state, thr_state)
n = size(A,1);
m = size(B,2);
...
%actuator dynamics
Z = [(2*h)/(g*T^2) 0               0 zeros(1,3);
     0             (2*h)/(g*T^2)   0 zeros(1,3);
     0             0               0 zeros(1,3);
     zeros(3,6)                     ];
 
Zm = [-h/(g*T^2)  0           0 zeros(1,3);
      0          -h/(g*T^2)   0 zeros(1,3);
      0           0           0 zeros(1,3);
      zeros(3,6)               ];
  
G = [h/(g*T^2) 0         0 zeros(1,3);
     0         h/(g*T^2) 0 zeros(1,3);
     0         0         0 zeros(1,3);
     zeros(3,6)           ];
...
%     end
    ...
    %actuator dynamics
    for i = 1 : N
        w01((i-1)*n+1 : i*n,:) = [ cos(x_tilt_current(3)+xr(3,cur_state)) sin(x_tilt_current(3)+xr(3,cur_state)) 0 zeros(1,3); 
                                  sin(x_tilt_current(3)+xr(3,cur_state)) cos(x_tilt_current(3)+xr(3,cur_state)) 0 zeros(1,3);
                                   0                      0                      1 zeros(1,3);
                                   zeros(3,6)                                                ] * [D/2;L/2;0;0;0;0] + h/g*[ddxr(1,cur_state+i); ddxr(2,cur_state+i); 0;0;0;0];
    end
    for i = 1 : N
        w02((i-1)*n+1 : i*n,:) = [ cos(x_tilt_current(3)+xr(3,cur_state)) sin(x_tilt_current(3)+xr(3,cur_state)) 0 zeros(1,3); 
                                  sin(x_tilt_current(3)+xr(3,cur_state)) cos(x_tilt_current(3)+xr(3,cur_state)) 0  zeros(1,3);
                                   0                      0                      1 zeros(1,3)
                                   zeros(3,6)                                                ] * [D/2;L/2;0;0;0;0] - h/g*[ddxr(1,cur_state+i); ddxr(2,cur_state+i); 0;0;0;0];
    end
