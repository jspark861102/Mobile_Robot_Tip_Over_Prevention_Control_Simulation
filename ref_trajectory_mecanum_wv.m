function [t, xr, ur, ddxr] = ref_trajectory_mecanum_wv(L, v_max, road_width,dt,T)
...
dx_ref = v_ref.*cos(theta_ref) - vy_ref.*sin(theta_ref);
dy_ref = v_ref.*sin(theta_ref) + vy_ref.*cos(theta_ref);
x_ref(1) = 0;
y_ref(1) = 0;
for i = 1 : length(t)-1
